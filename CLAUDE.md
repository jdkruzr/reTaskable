...
- `app/backend/src/main.rs` — `AppLoadBackend` impl, MSG dispatch, `sync()` (drains intake first), `compose_sync_response`, write handlers (`toggle_first` + the UID-addressed `toggle_by_uid`/`toggle_by_uid_inner`, `create`, `edit_first` + the UID-addressed `edit_by_uid`/`edit_by_uid_inner`, `delete_first` + the UID-addressed `delete_by_uid`/`delete_by_uid_inner`), `show_tasks` (JSON envelope, open/all filter), the M11 settings handlers (`get_config`, `save_config`/`save_config_inner` + `target_changed`, `discover_with`), and the M13 intake (`drain_intake` + testable `drain_intake_from`, `IntakeItem`/`make_anchor`)
...
- `app/ui/Main.qml` — scrollable `ListView` of tasks (pending-mark + checkbox + summary + M13 `📓 source` line), a fixed three-line status slot with the conditional Resolve-Conflict button beside it, a one-line action row (Sync · Settings · Show/Hide Completed · ▲/▼ discrete paging), Create field, the **task-detail dialog** (`openDetail`/`closeDetail` — single-tap a row → full-screen overlay with edit-summary + two-step delete, MSG 19/20), the M9b Keep/Take/Cancel row, the M11 Settings overlay (URL/username/password + calendar picker), and the M13 launch-time intake drain (MSG 18 → `applyDrainResult` loads the list)
