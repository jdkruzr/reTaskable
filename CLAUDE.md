...
- `app/backend/src/main.rs` — `AppLoadBackend` impl, MSG dispatch, `sync()`, `compose_sync_response`, write handlers (`toggle_first` + the UID-addressed `toggle_by_uid`/`toggle_by_uid_inner`, `create`, `edit_first`, `delete_first`), `show_tasks` (JSON envelope, open/all filter), and the M11 settings handlers (`get_config`, `save_config`/`save_config_inner` + `target_changed`, `discover_with`)
- `app/backend/src/db.rs` — SQLite schema + migration, cache CRUD, `enqueue_{create,edit,toggle,delete}`, `fetch_next_drainable`, `cascade_uid`, `clear_errored`, `pending_marks`, `reset_cache` (M11 target-switch wipe), `set_last_synced` (M12 token-less sync time), in-file tests
...
- `app/backend/src/nextcloud.rs` — HTTP helpers: discovery, `sync_collection_with_fallback` (sync-collection → `calendar_query_all` when `sync_collection_unsupported`), `put_task_with_retry` (412 auto-retry), `delete_task_with_retry`, iCalendar mutations (`toggle_completion`, `replace_summary`, `escape_ical_text`), task rendering (`format_tasks_json` live, `format_tasks_marked` test-only, `filter_for_display`)
...
- `app/ui/Main.qml` — scrollable `ListView` of tasks (pending-mark + checkbox + summary), ▲/▼ discrete paging, Show Completed toggle, Create field, Sync, the M9b Keep/Take/Cancel row, and the M11 Settings overlay (URL/username/password + calendar picker)
...
**MSG protocol.** Requests are 1–17, responses are 101–117, dense numbering so QML can filter with simple `||` chains. Reserved system types: `0xFFFFFFFE` (new-coordinator), `0xFFFFFFFF` (terminate). Frames are `{type:u32 LE, len:u32 LE}` + UTF-8 payload. Notable payloads: **MSG 4 / 104** carry a JSON task envelope (M10; request payload `"open"`/`"all"` selects the completed filter); **14 / 114** `toggle_by_uid` (request = uid; reply = compact `{uid,completed,mark}`); **15–17 / 115–117** settings (`get_config` / `save_config` / `discover_with`, JSON both ways — `get_config` returns `has_password` but never the secret); **112** (conflict preview) is multi-line with an `OPID:<n>\n` sentinel QML strips to drive the Keep/Take/Cancel row. Several response bodies are now JSON; QML `JSON.parse` failures are treated as plain status/error text (and clear the task list — see the M11 stale-list fix).
...
**Tests are in-file.** No separate `tests/` directory. Each module has its own `#[cfg(test)] mod tests { ... }` block using `Connection::open_in_memory()`. Pre-M9a there were 6 tests; M9a brought it to 54; M9b to 86; M9c to 94; M10 to 99; M11 to 107; M12 to 110.
