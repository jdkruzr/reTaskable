...
Latest milestone: **M13 — note-anchor intake (reTaskable receiving end of "lasso → to-do").** First step of capturing a reMarkable note selection as a CalDAV to-do that remembers where it came from. This milestone builds only the *receiving* end: an **intake spool** (`~/.local/share/retaskable/intake/*.json`) the backend drains on app launch (new MSG 18/118 `drain_intake`, fired by QML on load) and at the top of every `sync()`. Each file → an **anchored create**: `db::enqueue_create_with_anchor` stamps `X-RETASKABLE-SOURCE-{DOC,PAGE,LABEL}` into the VTODO. The label renders as a `📓 …` line under the to-do (parsed back via `nextcloud::extract_source_label`; surfaced in the MSG 4/104 envelope as a per-task `source` field). The anchor survives the offline-queue flush because the create payload now carries the full cached body and the queue PUTs it verbatim (`put_task_create_body`) instead of rebuilding from summary — so cache == what's sent == what the server stores (hardware-verified: all three X-props round-tripped through UltraBridge). The xochitl-side capture hook that *produces* intake files is the next milestone (M14); jump-back navigation is M15.
